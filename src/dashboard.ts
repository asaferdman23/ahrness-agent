...
export interface DashboardState {
  ...
  whatsappHomeChatKind: 'self' | 'group' | null; whatsappHomeChatSubject: string | null
...
export function dashboardWhatsappReady(input: {
  ...
  baileysHomeChatJid: string | null
}): boolean {
  if (!input.whatsappJid) return false
  if (input.provider !== 'baileys') return true
  return input.baileysConnected && !!input.baileysHomeChatJid
}
...
export function renderDashboardPage(user: User, state: DashboardState): string {
  ...
  const waNumber = state.whatsappJid?.split('@')[0]?.split(':')[0] ?? null
  const hasBaileysHomeChat = ready && state.whatsappProvider === 'baileys' && !!state.whatsappHomeChatSubject
  const hasBaileysHomeGroup = hasBaileysHomeChat && state.whatsappHomeChatKind === 'group'
  const hasBaileysSelfChat = hasBaileysHomeChat && state.whatsappHomeChatKind === 'self'
  const primaryHref = working ? '/dashboard/activity' : ready && waNumber && !hasBaileysHomeGroup ? `https://wa.me/${escapeHtml(waNumber)}` : `/onboarding/step/${Math.min(state.onboardingStep, 6)}`
  const primaryLabel = working ? 'See progress' : hasBaileysHomeGroup ? 'Open my BizzClaw group' : hasBaileysSelfChat ? 'Open Message yourself' : ready ? 'Open WhatsApp' : 'Continue setup'
  const primaryAction = hasBaileysHomeGroup && !working
    ? `<button class="btn btn-primary" id="open-home-group" type="button" onclick="openHomeGroup()">${primaryLabel}<span class="arrow" aria-hidden="true">→</span></button>`
    : `<a class="btn btn-primary" href="${primaryHref}"${ready && !working ? ' target="_blank" rel="noopener"' : ''}>${primaryLabel}<span class="arrow" aria-hidden="true">→</span></a>`
  const groupScript = hasBaileysHomeGroup
    ? `<script>async function openHomeGroup(){const button=document.getElementById('open-home-group');button.disabled=true;const old=button.innerHTML;button.textContent='Opening WhatsApp…';try{const response=await fetch('/api/whatsapp/home-chat-link',{method:'POST',headers:{'Accept':'application/json'}});const data=await response.json();if(!response.ok||!data.url)throw new Error(data.error||'Could not open the group');window.location.assign(data.url)}catch(error){button.innerHTML=old;button.disabled=false;alert(error instanceof Error?error.message:'Could not open the group. Please try again.')}}</script>`
    : ''
  ...
  const connections = [
    connectionCard('WhatsApp', state.whatsappLinked, state.whatsappLinked ? (state.whatsappHomeChatSubject ? `BizzClaw works in “${state.whatsappHomeChatSubject}”.` : 'Your primary place to send requests and receive results.') : 'Required to launch BizzClaw.', !state.whatsappLinked, state.whatsappProvider === 'baileys' && state.whatsappLinked ? { href: '/onboarding/step/5?manage=group', label: 'Change workspace' } : undefined),
    ...platformCards,
  ].join('')
