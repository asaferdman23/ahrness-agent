...
export interface DashboardState {
  ...
  whatsappHomeGroupSubject: string | null
...
export function renderDashboardPage(user: User, state: DashboardState): string {
  ...
  const hasBaileysHomeGroup = ready && state.whatsappProvider === 'baileys' && !!state.whatsappHomeGroupSubject
  ...
  const primaryLabel = working ? 'See progress' : hasBaileysHomeGroup ? 'Open my BizzClaw group' : ready ? 'Open WhatsApp' : 'Continue setup'
